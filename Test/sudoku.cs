...
namespace TestSudoku
{
    class Program
    {
        ...
        public bool genere_grille(int id_emplacement)
       {
            ...
            //valeurs possibles pour une ligne
            ...
            {
                possibilite_case[tmp] = valeurs_cases[line, col, tmp];
                if (valeurs_cases[line, col, tmp] == null)
                {
                    //réinitialisation OK
                    verif_possibilite_valeur = true;
                }
                if (valeurs_cases[line, col, tmp] == true)
                {
                    verif_possibilite_valeur = true;
                    nb_possibilite++;                
                }
            }
            if (verif_possibilite_valeur == true)//il ya des valeurs possibles
            {
                if (nb_possibilite == 0)
                {
                    for (int valeur_tmp = 1; valeur_tmp <= 9; valeur_tmp++)
                    {
                        if (nestpasdansligne(valeur_tmp, line, col) && nestpasdanscolonne(valeur_tmp, line, col) && nestpasdanscarre(valeur_tmp, line, col))
                        {
                            valeurs_cases[line, col, valeur_tmp - 1] = true;
                            possibilite_case[valeur_tmp - 1] = true;
                            nb_possibilite++;
                        }
                        //si la valeur est déja utilisée
                        else valeurs_cases[line, col, valeur_tmp - 1] = false;
                    }
                }
                if (nb_possibilite > 0)
                {
                    while ((verif_possibilite_nbchoix) == false)
                    {
